...
const PROMPT = `이 이미지는 초등학교/중학교에서 나눠주는 "주간학습계획표"입니다. 표를 읽어 아래 규칙대로 구조화하세요.

- 요일별로 각 교시의 과목명을 추출합니다. day 는 '월','화','수','목','금','토','일' 한 글자로.
- 각 교시에 start/end 시각을 "HH:MM"(24시간제)으로 채웁니다. 표에 시각이 적혀 있으면 그대로, 없으면 한국 초등학교 표준 시간표로 추정하세요(예: 1교시 09:00~09:40, 쉬는시간 10분, 4교시 후 점심 약 50분). period 는 교시 번호(정수).
- supplies: 그 요일의 준비물(준비물/가져올 것)을 한 줄로 정리. 없으면 빈 문자열.
- notes: 그 요일의 알림/안내/참고사항/숙제 등을 한 줄로 정리. 없으면 빈 문자열.
- 표에 등장하는 요일만 days 에 포함하세요. weekRange 는 등장한 요일에 맞춰 mon-fri / mon-sat / mon-sun 중 하나.
- 빈 칸/창의적체험활동 등 과목명이 불명확한 칸은 보이는 그대로(예: '창체')의 이름으로 넣되, 정말 비어 있으면 그 교시는 건너뜁니다.
